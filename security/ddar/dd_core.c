		...
		const char *func, struct dd_req *req)
{
	...
	__dd_debug(mask, func,
			...
			req->code, req->abort, req->pid, req->tgid, proc,
			req->state, req_time_spent(req), &req->list,
			req->list.prev, req->list.next, msg);
}
...
static struct dd_req *get_req(const char *msg, struct dd_info *info,
		enum dd_request_code_t code, int dir)
{
	struct dd_context *ctx = dd_context_global;
	struct dd_req  *req;
...
static inline void dd_proc_try_free(struct dd_proc *proc)
{
	...
	WARN_ON_ONCE(!proc->abort);
	if (reqcnt > 0) {
		dd_info("%s: delayed freeing proc:%p (reqcnt:%d)\n", __func__, proc, reqcnt);
	} else {
		int i;

		dd_info("%s: freeing proc:%p\n", __func__, proc);
...
static inline void put_req(const char *msg, struct dd_req *req)
{
	...
	if (!req->abort) {
		...
		WARN_ON_ONCE(req->state != DD_REQ_SUBMITTED);
	}
...
static inline void abort_req(const char *msg, struct dd_req *req, int err)
{
	struct dd_info *info = req->info;
	struct dd_proc *proc = (struct dd_proc *) info->proc;
	struct sdp_fs_command_t *cmd = NULL;
	...
	dd_error("%s - record audit log in case of a failure", __func__);
	if (req->dir == READ) {
	...
	case DD_REQ_SUBMITTED:
	{
		if (req->code == DD_REQ_CRYPTO_BIO) {
			unsigned int dir = bio_data_dir(req->u.bio.orig);
...
static struct dd_req *find_req(
		struct dd_proc *proc, enum dd_req_state_t state, unsigned long ino)
__acquires(proc->lock)
__releases(proc->lock)
{
...
static struct bio *dd_clone_bio(struct dd_req *req, struct bio *orig, unsigned int size)
{
	struct dd_info *info = req->info;
	struct dd_context *ctx = (struct dd_context *) info->context;
	struct bio *clone;
...
static int __request_user(struct dd_req *req)
{
	...
	WARN_ON_ONCE(!req->user_space_crypto);
	WARN_ON_ONCE(!info || !context);
...
static void dd_end_io(struct bio *clone)
{
	...
	struct dd_info *info = req->info;
	struct bio *orig = req->u.bio.orig;
	unsigned int dir = bio_data_dir(clone);
	int err;
...
int dd_submit_bio(struct dd_info *info, struct bio *bio)
{
	...
	if (bio_data_dir(bio) == WRITE) {
		...
		if (req->user_space_crypto) {
			...
			if (dd_sec_crypt_bio_pages(
					info, req->u.bio.orig, req->u.bio.clone, DD_ENCRYPT)) {
				dd_error("failed dd crypto\n");
				goto err_out;
			}
		...
		struct page *src_page, struct page *dst_page)
{
	...
	WARN_ON_ONCE(!src_page || !dst_page);
...
struct dd_mmap_layout default_mmap_layout = {
		...
		.metadata_area = {
				.start = 0x01000000,
				.size = PAGE_SIZE * MAX_NUM_INO_PER_USER_REQUEST }, // 128KB
		.plaintext_page_area = {
				.start = 0x02000000,
				.size = PAGE_SIZE * LIMIT_PAGE_NUM }, // 1 MB
		.ciphertext_page_area = {
				.start = 0x03000000,
...
static inline struct dd_mmap_control *__get_next_ctr_page(struct dd_transaction *t)
{
	...
	dd_memory("mapping ctr page proc:%p control:%p [mapped pages:%d] [total ctr pages:%d]\n",
			proc,
			(void *) (control_area_addr + (t->num_ctr_pg << PAGE_SHIFT)),
			t->num_mapped_pg, t->num_ctr_pg);

	WARN_ON_ONCE(t->num_ctr_pg > MAX_NUM_CONTROL_PAGE);
	return control;
}
...
static inline void __get_md_page(struct dd_proc *proc, struct page *mdpage,
		unsigned long ino, struct dd_transaction *t)
{
	int i = 0;
...
#ifdef CONFIG_DDAR_USER_PREPARE
static enum dd_transaction_result __process_prepare_request_locked(
...
static enum dd_transaction_result __process_bio_request_locked(
		struct dd_req *req, struct dd_transaction *t)
{
	struct dd_proc *proc = t->proc;
	...
	unsigned int require_ctr_pages;
...
//			MAX_NUM_CONTROL_PAGE - t->num_ctr_pg, require_ctr_pages);
	...
	while (orig->bi_iter.bi_size) {
		...
		if (dir == WRITE) {
			...
			WARN_ON_ONCE(req->info->ino != orig_bv.bv_page->mapping->host->i_ino);
			...
			remap_pfn_range(proc->ciphertext_vma,
					p->u.bio.cipher_addr, page_to_pfn(ciphertext_page),
					PAGE_SIZE, proc->ciphertext_vma->vm_page_prot);
		...
		dd_memory("remap_pfn_range(plain:%p cipher:%p) dir:%d t->num_mapped_pg:%d\n",
				(void *)p->u.bio.plain_addr, (void *)p->u.bio.cipher_addr,
				dir, t->num_mapped_pg);
	}
...
static enum dd_transaction_result __process_page_request_locked(
		struct dd_req *req,  struct dd_transaction *t)
{
	struct dd_proc *proc = t->proc;
	int vm_offset = t->num_mapped_pg << PAGE_SHIFT;
	struct dd_user_req *p = NULL;
	enum dd_crypto_direction_t dir = req->u.page.dir;
	unsigned int require_ctr_pages = 1;
...
__acquires(proc->lock)
{
	...
	list_for_each_entry_safe(req, temp, &proc->pending, list) {
		enum dd_transaction_result result = DD_TRANS_ERR_UNKNOWN;
		...
		dd_verbose("[unique:%d][code:%d][ino:%ld][num_pg:%d][num_md:%d][num_ctr:%d]\n",
				req->unique, req->code, req->ino,
				t->num_mapped_pg, t->num_md_pg, t->num_ctr_pg);
		switch (req->code) {
		case DD_REQ_PREPARE:
		{
#ifdef CONFIG_DDAR_USER_PREPARE
...
long dd_ioctl_wait_crypto_request(struct dd_proc *proc, struct dd_ioc *ioc)
{
	...
	if (!dd_mmap_available(proc)) {
		dd_error("%s: user space memory not registered\n", __func__);
		return -ENOMEM;
	}
	...
	if (list_empty(&proc->pending)) {
		...
		while (!proc->abort && list_empty(&proc->pending)) {
			...
			if (signal_pending(current)) {
				dd_error("received signal in interruptible process state\n");
				rc = -EINTR;
				break;
			}
	...
	if (rc) {
		dd_error("%s failed, try again :%d\n", __func__, rc);
		return rc;
	}

	dd_verbose("%s: retrieving req pid:%p\n", __func__, proc);
	...
	list_for_each_entry_safe(req, temp, &proc->processing, list) {
		...
		if (err) {
			...
			case DD_REQ_CRYPTO_BIO:
			{
				...
				if (dir == WRITE) {
					dd_dump_bio_pages(
							"inner encryption done", req->u.bio.clone);
...
static void dd_write_metadata_work(struct work_struct *work)
{
	...
	dd_verbose("%s - %ld complete (i_state:0x%x)\n", __func__,
...
static long dd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case DD_IOCTL_SET_XATTR:
	{
		...
		if (!igrab(ioc_work->inode)) {
			dd_error("failed to grab inode refcount, may be getting removed\n");
			kfree(ioc_work);
			return 0;
		}
...
struct dd_info *alloc_dd_info(struct inode *inode,
				const struct dd_policy *dd_policy,
				struct dd_crypt_context *crypt_context)
{
	struct dd_info *info;
	...
	if (!dd_policy) {
		dd_error("%s: dd_policy is null (ino:%ld)\n", __func__, inode->i_ino);
		return ERR_PTR(-EINVAL);
	}

	WARN_ON_ONCE(!ext4_dd_info_cachep);
	...
	if (S_ISREG(inode->i_mode)) {
		if (dd_policy_user_space_crypto(dd_policy->flags)) {
			...
			if (rc) {
				dd_error("can't find master key for user:%d\n", dd_policy->userid);
				return ERR_PTR(-ENOKEY);
			}
		}
	}
	...
	if (dd_policy_user_space_crypto(dd_policy->flags)) {
		...
		if (crypt_context == NULL) {
			...
			memcpy(&info->crypt_context, &temp_crypt_context,
					sizeof(struct dd_crypt_context));
		} else {
			memcpy(&info->crypt_context, crypt_context,
					sizeof(struct dd_crypt_context));
		}
...
void dd_init_context(const char *name)
{
	struct dd_context *context =
			kmalloc(sizeof(struct dd_context), GFP_KERNEL);

	if (context) {
		int ret = 0;

		dd_info("%s - %s\n", __func__, name);
		strscpy(context->name, name, sizeof(context->name) - 1);
...
//		abort_thread = kthread_run(
//				abort_req_timeout_thread, NULL, "ddar_abort_req_timeoutd");
//		if (abort_thread) {
...
static int __init dd_init(void)
{
	...
	WARN_ON_ONCE(sizeof(struct metadata_hdr) != METADATA_HEADER_LEN);
